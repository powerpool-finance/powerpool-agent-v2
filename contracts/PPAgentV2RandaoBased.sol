...
import {PPAgentV2Based, ConfigFlags } from "./PPAgentV2Based.sol";
import "./utils/CustomizedEnumerableSet.sol";
import "./PPAgentV2Flags.sol";
import "./PPAgentV2Interfaces.sol";

/**
 * @title PPAgentV2RandaoBased
 * @author PowerPool
 */
contract PPAgentV2RandaoBased is IPPAgentV2RandaoViewer, PPAgentV2Based {
  ...
  function initializeRandao(
    ...
    PPAgentV2Based.initialize(owner_, minKeeperCvp_, pendingWithdrawalTimeoutSeconds_);
  ...
  function initiateKeeperSlashing(
    ...
    if (useResolver_) {
      ...
      (bool ok, bytes memory result) = address(this).call(
        abi.encodeWithSelector(PPAgentV2Based.checkCouldBeExecuted.selector, jobAddress_, jobCalldata_)
      );
      if (ok) {
