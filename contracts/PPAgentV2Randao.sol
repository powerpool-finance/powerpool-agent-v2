...
import { ConfigFlags, IPPAgentV2Executor, IPPAgentV2Viewer } from "./PPAgentV2.sol";

/**
 * @title PowerAgentLite
 * @author PowerPool
 */
contract PPAgentV2Randao is IPPAgentV2Executor, IPPAgentV2Viewer, PPAgentV2Flags, Ownable {
  ...
  mapping(bytes32 => uint256) internal jobNextKeeperId;
  ...
  function execute_44g58pv() external {
    ...
    // -1. Keeper is eligible to call this
    require(jobNextKeeperId[jobKey] == keeperId, "wrong keeper");
    ...
    keeperLockByJob[keeperId].remove(jobKey);
    _assignNextKeeper(jobKey);
  }

  uint256 public nextExecutionKeeperId;
  mapping(uint256 => EnumerableSet.Bytes32Set) internal keeperLockByJob;
  ...
  function _assignNextKeeper(bytes32 _jobKey) internal {
    ...
    keeperLockByJob[_nextExecutionKeeperId].add(_jobKey);
  }
  ...
  function registerJob(
    ...
    if (lastKeeperId == 0) {
      revert("neet at leas one keeper");
    }
  ...
  function initiateRedeem(uint256 keeperId_, uint256 amount_) external returns (uint256 pendingWithdrawalAfter) {
    ...
    if (keeperLockByJob[keeperId_].length() > 0) {
      revert ("at least one job is assigned");
    }
