...
import { DsMath, DsMath32, DsMath112, DsMath224 } from "./DsMath.sol";
import "./Interfaces.sol";

contract BaseRewardPool is ReentrancyGuardUpgradeable, OwnableUpgradeable, IBaseRewardPool {
    using DsMath for uint256;
    using DsMath224 for uint224;
    using DsMath112 for uint112;
    using DsMath32 for uint32;
    ...
    ****************************************/
     ...
     * @param _stakingToken     CVX
     * @param _cvxCrv           cvxCrv
     * @param _cvxCrvStaking    cvxCrv rewards
     */
    function initialize(
    ...
    ****************************************/
    ...
    function _lastTimeRewardApplicable(uint256 _finishTime) internal view returns (uint256) {
        return DsMath.min(block.timestamp, _finishTime);
    }
    ...
    function _processExpiredLocks(
        ...
        if (isShutdown || locks[length - 1].unlockTime <= expiryTime) {
            ...
            if (_checkDelay > 0) {
                ...
                uint256 rRate = DsMath.min(kickRewardPerEpoch.mul(epochsover + 1), denominator);
    ...
    ****************************************/
    ...
    returns (DelegateeCheckpoint memory)
    {
        ...
        uint256 low = 0;
        while (low < high) {
            uint256 mid = DsMath.average(low, high);
